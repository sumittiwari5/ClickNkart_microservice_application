...
*/}}
{{- define "user-service.versionedDeployment" -}}
{{- $root := .root -}}
{{- $version := .version -}}
apiVersion: apps/v1
kind: Deployment
metadata:
  name: user-service-v{{ $version }}
  labels:
    app: user-service
    version: "v{{ $version }}"
  {{- if .retiredAt }}
...
{{- define "user-service.versionedHPA" -}}
{{- $root := .root -}}
{{- $version := .version -}}
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: user-service-v{{ $version }}-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
...
{{- define "user-service.versionedPDB" -}}
{{- $version := .version -}}
apiVersion: policy/v1
kind: PodDisruptionBudget
metadata:
  name: user-service-v{{ $version }}-pdb
spec:
  minAvailable: 1
  selector:
    matchLabels:
