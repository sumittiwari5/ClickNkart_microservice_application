...
*/}}
{{- define "order-service.versionedDeployment" -}}
{{- $root := .root -}}
{{- $version := .version -}}
apiVersion: apps/v1
kind: Deployment
metadata:
  name: order-service-v{{ $version }}
  labels:
    app: order-service
    version: "v{{ $version }}"
  {{- if .retiredAt }}
...
{{- define "order-service.versionedHPA" -}}
{{- $root := .root -}}
{{- $version := .version -}}
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: order-service-v{{ $version }}-hpa
spec:
  scaleTargetRef:
    apiVersion: apps/v1
    kind: Deployment
...
{{- define "order-service.versionedPDB" -}}
{{- $version := .version -}}
apiVersion: policy/v1
kind: PodDisruptionBudget
metadata:
  name: order-service-v{{ $version }}-pdb
spec:
  minAvailable: 1
  selector:
    matchLabels:
